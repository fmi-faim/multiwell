from tkinter import LabelFrame, Frame, Checkbutton, Button, Entry, Label, Tk, StringVar, DoubleVar, IntVar, Scale, DISABLED, BOTH, CENTER, HORIZONTAL, NORMAL, END, RIDGE, RAISED, SUNKEN, W, LEFT, messagebox
...
CameraFormats = {"PCO.Edge":(6.5, 2048, 2048), "CascadeII":(16, 512, 512), "Prime95":(11, 1200, 1200), "iXon 888":(13, 1024, 1024), "Other":("", "", "")}
...
def clickGeneral(ObjectClicked):
    if ObjectClicked["state"] == DISABLED: pass
    else:
        if ObjectClicked["bg"] == "SystemButtonFace": ObjectClicked.config(bg="White", fg="Blue")
        else: ObjectClicked.config(bg="SystemButtonFace", fg="SystemButtonText")
        ...
        if numWell[i] == event.widget:
            ...
            for i in range(0, 16):
                for j in range(0, 24):
                    ...
                    counter = counter + 1
        else: numWell[i].config(bg="SystemButtonFace", fg="SystemButtonText")
...
def SelectCamera(event):
    for i in range(0, len(Cameras)):
        ...
        else: CameraList[i].config(bg="SystemButtonFace", fg="SystemButtonText")
...
def clickTile(event):
    ...
    for i in range(0, len(PossNumTiles)):
        ...
        if event.widget["text"] == Text:
            ...
            Tile = i + 1
        else: numTile[i].config(bg="SystemButtonFace", fg="SystemButtonText")
...
#   Needs to get all values in all fields at this step
    ...
    if optovar.get() == "None": OPTOVAR = 1
    else: OPTOVAR = float(optovar.get())
    ...
    PositionList.write("\"Stage Memory List\", Version 6.0 \n0, 0, 0, 0, 0, 0, 0, \"um\", \"um\" \n0 \n"+str(CountPos)+"\n")
    CountPos = 0
    for j in range(0, formate[1]):
        for i in range(0, formate[0]):
            Text = WellButtonList[j * 24 + i]
            if Text["bg"] == "White":
                ...
                for k in range(0, Tile):
                    for l in range(0, Tile):
                        CountPos = CountPos + 1
                        PositionList.write("\"Position "+Text["text"]+"_"+str(k+1)+"_"+str(l+1)+"\", "+("%.3f" % XTile)+", "+("%.3f" % YTile)+", "+("%.3f" % Z)+", 0, 0, FALSE, -9999, TRUE, TRUE, 0, -1, \"\"\n")
...
def clickRow(event):
    ...
    for i in range(0, 24):
        ...
        if tempo["state"] == NORMAL: WellButtonList[col.index(event.widget["text"]) * 24 + i].config(bg=event.widget["bg"], fg=event.widget["fg"])

def clickCol(event):
    clickGeneral(event.widget)
    for i in range(0, 16):
        tempo = WellButtonList[i * 24 + int(event.widget["text"])-1]
        if tempo["state"] == NORMAL: WellButtonList[i * 24 + int(event.widget["text"])-1].config(bg=event.widget["bg"], fg=event.widget["fg"])
...
numTile = []
for i in range(0, len(PossNumTiles)):
    numTile.append(Button(root, text=str(PossNumTiles[i]) + "x" + str(PossNumTiles[i]), width=4, height=1, justify=CENTER))
...
overlap = Scale(root, length=200, fg="Blue", troughcolor="White", activebackground="Blue", orient=HORIZONTAL, resolution=5)
overlap.set(10)
...
for i in range(0, 16):
    for j in range (0, 24):
        WellButtonList.append(Button(root, text=col[i] + str(j + 1), width=3, height=1, justify=CENTER, disabledforeground="White", state=NORMAL))
